  ...
  try {
    ...
    try {
      ...
      if (!Array.isArray(oneriler)) {
        ...
        if (firstArray) {
          oneriler = firstArray as MesajOnerisi[];
        } else {
      ...
      oneriler = oneriler
        .filter(
          (o) =>
            typeof o?.mesaj === "string" && typeof o?.aciklama === "string"
        )
        .slice(0, 3)
        .map((o) => ({
          mesaj: o.mesaj.trim(),
      ...
      if (oneriler.length < 2) throw new Error("Yetersiz öneri sayısı");
    } catch (e: unknown) {
      console.error("JSON parse hatası:", e, "icerik:", icerik);
      return NextResponse.json(
        { hata: "AI'ın yanıtı anlaşılamadı. Tekrar dene, genelde düzelir." },
        { status: 500 }
      );
    }
